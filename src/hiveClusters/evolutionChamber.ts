...
export class EvolutionChamber extends HiveCluster {
	...
	private initLabStatus(): void {
		...
		const reagents: ResourceConstant[] =
			this.memory.activeReaction ?
	...
	private registerReagentLabRequests(
	...
	private registerRequests(): void {
		...
		this.registerReagentLabRequests(
	...
	run(): void {
		...
		if (!this.memory.activeReaction && !this.memory.suspendReactionsUntil) {
			...
			if (nextReaction) {
				...
				const sleepTime =
					EvolutionChamber.settings.sleepTime + randint(0, 20);
				log.info(
