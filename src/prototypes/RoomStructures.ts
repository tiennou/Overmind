...
const roomStructureIDs: {
	[roomName: string]: { [structureType: string]: string[] };
} = {};
		...
		get: function (this: Room) {
			...
			if (val) {
				...
				if (roomStructureIDs[this.name][type]) {
					val = _.compact(
						_.map(roomStructureIDs[this.name][type], (id) =>
...
Object.defineProperty(Room.prototype, "storageUnits", {
	get(this: Room) {
		if (!this._storageUnits) {
			const special: (StorageUnit | undefined)[] = [
				this.storage,
				this.terminal,
			];
			this._storageUnits = <StorageUnit[]>(
				_.compact(special).concat(<StorageUnit[]>this.containers)
			);
		}
		...
		if (!this._repairables) {
			...
			if (roomStructureIDs[this.name].repairables) {
				return (this._repairables = _.compact(
					_.map(roomStructureIDs[this.name].repairables, (o) =>
						Game.getObjectById<Structure>(o)
					)
				));
			} else {
				...
				for (const structureType of multipleList) {
					if (
						...
						const obj = <
							{ [p: string]: Structure<StructureConstant>[] }
						>(<any>this);
		...
		if (!this._walkableRamparts) {
			...
			if (roomStructureIDs[this.name].walkableRamparts) {
				return (this._walkableRamparts = _.compact(
					_.map(roomStructureIDs[this.name].walkableRamparts, (o) =>
						Game.getObjectById<StructureRampart>(o)
					)
				));
			} else {
...
Object.defineProperty(Room.prototype, "barriers", {
	get(this: Room) {
		if (!this._barriers) {
			this._barriers = (<(StructureRampart | StructureWall)[]>[]).concat(
				this.ramparts,
				this.constructedWalls
			);
		}
