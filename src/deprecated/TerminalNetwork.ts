...
export class TerminalNetwork /*implements ITerminalNetwork*/ {
	...
	private getAllAssets(): { [resourceType: string]: number } {
		const p = <StoreContents[]>(
			_.map(this.terminals, (terminal) =>
				!!colonyOf(terminal) ? colonyOf(terminal).assets : {}
			)
		);
		return mergeSum(...p);
	}
	...
	private terminalNeedsEnergy(terminal: StructureTerminal): boolean {
		return (
			terminal.energy < Energetics.settings.terminal.energy.inThreshold
		);
	}
		...
		for (const resource in terminal.store) {
			...
			if (resource == RESOURCE_ENERGY) {
				...
				if (terminalNearCapacity) {
					// if you're close to full, be more aggressive with selling energy
					energyThreshold =
	...
	private equalize(
		...
		const maxSendSize =
			resourceType == RESOURCE_ENERGY ?
				TerminalNetwork.settings.equalize.maxEnergySendAmount
		...
		if (verbose)
			log.debug(
				_.map(
					terminalsByResource,
					(t) => `${t.room.name}: ${colonyOf(t).assets[resourceType]}`
				)
			);
	...
	private equalizeCycle(): void {
		...
		const resourceEqualizeOrder = cyclicListPermutation(
			equalizeResources,
			this.memory.equalizeIndex + 1
		);
	...
	handleOverflowWithNoEnergy() {
		for (const terminal of this.terminals) {
			if (
				terminal.store.getFreeCapacity() < 5000 &&
				terminal.store[RESOURCE_ENERGY] < 10000
			) {
				const roomOrders = _.filter(
					Game.market.orders,
					(order) => order.roomName == terminal.room.name
				);
				for (const res in terminal.store) {
					const quantity = terminal.store[res as ResourceConstant];
					if (
						quantity > 15000 &&
						roomOrders.filter((order) => order.resourceType == res)
							.length == 0
					) {
						log.info(
							`Creating sell order for ${res} in room ${terminal.room.print}`
						);
						Overmind.tradeNetwork.sell(
							terminal,
