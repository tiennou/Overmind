...
export class GatheringOverlord extends Overlord {
	...
	constructor(directive: DirectiveGather, priority: number) {
		...
		this.harvestPos =
			this.container ? this.container.pos : this.calculateContainerPos();
	}
			...
			for (const pos of neighbors) {
				...
				for (const struct of structures) {
					...
					const sharedNeighbors = structNeighbors.filter(
						(structNeighbor) => {
							return neighbors.some((neighporPos) =>
								neighporPos.isEqualTo(structNeighbor)
							);
						}
					);
	...
	private miningActions(miner: Zerg) {
		...
		if (
			!this.deposit ||
			(this.deposit instanceof Deposit && !this.container)
		) {
			return true;
		}
		...
		this.debug(
			`${miner.print} gathering from ${this.deposit.print}: ${result}`
		);
		...
		if (result === OK) {
			...
			this.debug(
				`${miner.print} not actually in range, moving closer to ${this.deposit.print}`
			);
	...
	private handleTransfer(miner: Zerg, emptyOut?: boolean) {
		...
		if (!overfilled && !(emptyOut || miner.store.getUsedCapacity() === 0)) {
			return false;
		}
