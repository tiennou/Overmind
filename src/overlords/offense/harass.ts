...
export class HarassOverlord extends CombatOverlord {
	...
	private handleHarass(hydralisk: CombatZerg): void {
		...
		if (
			...
			hydralisk.room.dangerousHostiles.forEach((hostile) => {
				const nextSafeSpawn =
					Game.time -
					HarassOverlord.settings.prespawn +
			...
			if (hydralisk.room.name != this.targetRemoteToHarass) {
				...
				const nextRoom = this.chooseRemoteToHarass(
					hydralisk,
					hydralisk.room.name
				);
				if (nextRoom) {
	...
	private chooseRemoteToHarass(hydralisk: CombatZerg, _currentRoom: string) {
		...
		if (nextRoom) {
			...
			log.debug(
				`Selecting new target of ${this.targetRemoteToHarass} for ${hydralisk.print} from ` +
					`${this.directive.memory.roomsToHarass}`
			);
