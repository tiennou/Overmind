...
export class DirectiveBaseOperator extends Directive {
	...
	constructor(flag: Flag) {
		...
		if (!powerCreep) {
			log.error(
				`Power Creep not found for ${this.print}, deleting directive`
			);
			this.remove();
		}
	...
	renew(
		powerCreep: PowerCreep,
		powerSource: StructurePowerBank | StructurePowerSpawn
	) {
	...
	run(): void {
		...
		const _isStationary = powerCreep.name
			.toLowerCase()
			.indexOf(types.basedefender.toString());
		if (powerCreep.name == "activate") {
			console.log(
				"Power creep move is " + JSON.stringify(powerCreep.memory)
			);
		}
		...
		if (
			this.room.hostiles.length > 2 ||
			(powerCreep.pos &&
				DirectiveNukeResponse.isPresent(powerCreep.pos.roomName))
		) {
			const towersToBoost = this.colony.towers.filter(
				(tower) => !tower.effects || tower.effects.length == 0
			);
			if (towersToBoost.length > 0) {
				powerCreep.usePower(PWR_OPERATE_TOWER, towersToBoost[0]);
			}
			if (
				(!powerCreep.carry.ops || powerCreep.carry.ops < 20) &&
				this.room.storage &&
