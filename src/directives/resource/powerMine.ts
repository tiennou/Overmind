...
export class DirectivePowerMine extends Directive {
	...
	manageState() {
		...
		if (
			currentState == PowerMineState.init &&
			this.powerBank &&
			...
			if (
				...
				log.alert(
					`Power bank mining ${this.print} competing with ${this.powerBank.room.hostiles[0].owner.username}.`
				);
				// this.remove();
			} else {
			...
			currentState == PowerMineState.miningDone &&
			this.room &&
			this.pos.isVisible &&
			!this.hasDrops &&
			...
			currentState == PowerMineState.haulingComplete &&
			this.overlords.powerHaul &&
			(
				this.overlords.powerHaul as PowerHaulingOverlord
			).checkIfStillCarryingPower() == undefined
		) {
			// TODO Doesn't give enough time to pick up power
			log.notify(
				`Hauling complete for ${this.print} at time ${Game.time}. Final power collected was ` +
	...
	init(): void {
		...
		if (this.pos.room && !!this.powerBank) {
			alert = `PM ${powerMineState(this.memory.state)} ${
				this.totalResources
			} P${Math.floor(
				(100 * this.powerBank.hits) / this.powerBank.hitsMax
			)}% @ ${this.powerBank.ticksToDecay}TTL`;
		} else {
			alert = `PowerMine ${powerMineState(this.memory.state)} ${
				this.totalResources
			}`;
		}
		this.alert(alert);
	...
	run(): void {
		...
		const frequency =
			this.memory.state == PowerMineState.haulingStarted ? 1 : 21;
