...
const check_cfg: (cfg: Codec) => void = (cfg: Codec) => {
	...
	if (!fail) {
		(() => {
			...
			const fail_depth = (x: number | any) =>
				isNaN(x) || x <= 0 || x > MAX_DEPTH;
			if (depth_is_array) {
...
const serialize_meta: (str: string, meta: Codec) => string = (str, meta) => {
...
const deserialize_meta: (
	str: string,
	meta: Codec,
	offset?: number
) => [string, number] = (str, meta, offset) => {
	offset = offset || 0;
...
function encode_array(res: string, values: number[]) {
	...
	assert(
		fixed_depth || depths.length === values.length,
		TypeCodecError,
		"Wrong depths array length:",
		depths,
		values
	);
...
function decode_array(str: string, meta: Codec) {
	assert(
		!this.meta ||
			meta.depth > 0 ||
			(meta.depth === 0 && Array.isArray(this.depth)),
		TypeCodecError,
	...
	const length =
		depth_is_array ?
			(<number[]>meta.depth).length
		:	code_point_to_num(<number>str.codePointAt(it++));
...
export class Codec {
	...
	encode(arg: number | number[]): string {
		// @ts-ignore
		assert(
			(+Array.isArray(arg) | +!!arg.BYTES_PER_ELEMENT) ^ !this.array,
			TypeCodecError,
	...
	decode(str: string, length_out?: any) {
		...
		// @ts-ignore
		assert(
			meta.array ^ !this.array,
			TypeCodecError,
			"Incompatible codec (array <=> single value), str =",
			str
		);
...
export function testEncoder() {
	...
	for (let k = 0, biglen = idsPacked.length; k < biglen; ++k) {
		idsUnpacked.push(
			idCodec
				.decode(idsPacked[k])
				.reduce((acc: string, x: number) => acc + x.toString(16), "")
		);
	}
	elapsed = Game.cpu.getUsed() - start;
	console.log(
	...
	console.log(
		`Unpacked len: ${JSON.stringify(allPos).length} | Packed len: ${
			JSON.stringify(posPacked).length
		}`
	);
