 ...
 */
function verifyPowerCreepSetup(
	role: string,
	setup: PowerCreepSetup,
	verbose = false
): void {
	...
	while (true) {
		...
		if (upgradePower) {
			...
			if (verbose) {
				log.info(
					`Upgrading power ${upgradePower.power} to level ${
						powerLevels[upgradePower.power] + 1
					}, ` + `Total level: ${level + 1}`
				);
			}
			...
			for (const powerPriority of priorities) {
				...
				if (powerLevels[power] != maxLevel) {
					log.error(
						`Invalid power creep setup: ${role}! Power ${power} at level ${powerLevels[power]}, ` +
