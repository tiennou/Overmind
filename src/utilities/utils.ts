...
export function deref(ref: string): RoomObject | null {
	return (
		(Game.getObjectById<any>(ref) as any as RoomObject) ||
		Game.flags[ref] ||
		Game.creeps[ref] ||
		Game.spawns[ref] ||
		null
...
export function hasMinerals(store: StoreDefinition): boolean {
	for (const resourceType in store) {
		if (
			...
			(store[<ResourceConstant>resourceType] || 0) > 0
		) {
			return true;
		}
	}
...
export function hasContents(store: {
...
export function mergeSum(...stores: StoreContents[]): StoreContents {
	...
	for (const store of stores) {
		for (const [key, amount] of <[ResourceConstant, number][]>(
...
export function posFromReadableName(
