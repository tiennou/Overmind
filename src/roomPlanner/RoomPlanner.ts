...
export class RoomPlanner {
	...
	plannedStructurePositions(
		...
		const roomMap =
			this.memory.mapsByLevel ? this.memory.mapsByLevel[8] : undefined;
	...
	private generatePlan(level = 8): RoomPlan {
		...
		for (const name in this.placements) {
			...
			if (layout) {
				...
				const pos =
					this.placements[
						<"hatchery" | "commandCenter" | "bunker">name
					];
				if (!pos) {
					continue;
				}
	...
	private mapFromPlan(plan: RoomPlan): StructureMap {
		...
		const structureNames: string[] = _.unique(
	...
	private destroyStructure(structure: Structure) {
		if (Memory.settings.roomPlanner.allowDestroy) {
			...
			log.warning(
				`${this.colony.print}: RoomPlanner would have deleted ${structure.print} but destruction is disabled in settings`
			);
			return OK;
		}
	}
			...
			for (const structure of structures) {
				if (
					...
					if (
						this.colony.level < 6 &&
						structureType == STRUCTURE_TERMINAL &&
						hasMinerals((<StructureTerminal>structure).store)
					) {
			...
			if (this.map[structureType]) {
				for (const pos of this.map[structureType]) {
					if (
						...
						RoomPlanner.canBuild(structureType, pos)
					) {
						const result =
						...
						if (result === ERR_FULL || result === ERR_NOT_OWNER) {
							...
							for (const structure of structures) {
								...
								if (
									...
									if (result == OK) {
										log.info(
											`${this.colony.name}: destroyed ${structure.structureType} at ${structure.pos.print}`
										);
										this.memory.recheckStructuresAt =
											Game.time +
	...
	private nextNeededLinkAnchor(): RoomPosition | undefined {
		const linksEtAl = _.map(
			(<(StructureLink | ConstructionSite)[]>[]).concat(
				this.colony.links,
				_.filter(
	...
	shouldRecheck(offset = 0): boolean {
		if (
			...
			const checkFreq =
				RoomPlanner.settings.siteCheckFrequency * this.colony.level;
			return (
				Game.time % checkFreq ==
