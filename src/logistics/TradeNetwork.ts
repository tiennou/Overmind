...
export class TraderJoe {
	...
	private computeEffectiveEnergyPrices(): void {
		...
		for (const colony of _.sample(getAllColonies(), 5)) {
			...
			const _sellDirectPrice = maxBy(
				buyOrders,
				(order) =>
					order.price - this.marginalTransactionPrice(order, room)
			);
			const _buyDirectPrice = minBy(
				sellOrders,
				(order) =>
					order.price + this.marginalTransactionPrice(order, room)
			);
			const _sellOrderPrice = this.computeCompetitivePrice(
				ORDER_SELL,
				RESOURCE_ENERGY,
				room
			);
			const _buyOrderPrice = this.computeCompetitivePrice(
				ORDER_BUY,
				RESOURCE_ENERGY,
				room
			);
	...
	private getPriceForBaseIngredients(resource: ResourceConstant): number {
		...
		if (ingredients.length > 0) {
			...
			return _.sum(
				ingredients,
				(res) => this.memory.cache.history[res]?.avg14 ?? Infinity
			);
		} else {
			// not synthesizeable
			...
			if (!normalFluctuation && Math.random() < 1 / 2000) {
				...
				this.notify(
					`${terminal.room.print}: changing ${type} order price for ${resource} from ` +
						`${
							existingOrder.price
						} to ${price}. Response: ${errorForCode(ret)}`
				);
				return ret;
			}
		...
		const ordersToClean = _.filter(Game.market.orders, (order) => {
			...
			if (
				Game.time - order.created >
					TraderJoe.settings.market.orders.timeout &&
	...
	private buyDirect(
		...
		const minAmount =
			opts.flexibleAmount ?
				Math.min(
					TraderJoe.settings.market.orders.minBuyDirectAmount,
					amount
				)
			:	amount;
		...
		const adjustedPrice =
			order.price +
			this.marginalTransactionPrice(order, terminal.room.name);
		const priceForBaseIngredients =
			this.getPriceForBaseIngredients(resource);
	...
	private sellDirect(
		...
		const minAmount =
			opts.flexibleAmount ?
				Math.min(
					amount,
					TraderJoe.settings.market.orders.minSellDirectAmount
				)
			:	amount;
		const validOrders = _.filter(
			Game.market.getAllOrders({
		...
		if (
			...
			this.notify(
				`Buy direct request: ${amount} ${resource} to ${printRoomName(
					...
					`was unsuccessful (${errorForCode(
						result
					)}); allowing fallthrough to TradeNetwork.maintainOrder()`
			);
		}
